...
Las cadenas de texto _**\(strings\)**_ son uno de los tipos de datos más importantes en la programación. En muchas formas se comportan similar a los arreglos \(piensa en una cadena de texto como un arreglo de caracteres\), y a veces es útil convertir de cadenas de texto a arreglos y viceversa. En este capítulo veremos todos los aspectos interesantes que podemos hacer con cadenas de texto.
...
El método `split` **recibe el carácter por el que quieres partir la cadena.** Por ejemplo, podemos partir una cadena por comas:
...
El método `join` recibe un argumento que es el carácter que se va a utilizar para separar los elementos. Por ejemplo, podemos unir un arreglo con guiones:
...
**Puedes obtener una porción de la cadena con el método `substr`** que recibe dos argumentos: el índice desde el cuál se va a retornar la subcadena y, de forma opcional, el número de caracteres que se quiere extraer. Si se omite el segundo argumento se extrae hasta el final de la cadena:
